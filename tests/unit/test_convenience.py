...
from mermaid_render.convenience import quick_render, render_to_file, render
...
class TestQuickRender:
    ...
    def test_basic_svg_rendering(self) -> None:
        ...
        with patch('mermaid_render.convenience.MermaidRenderer') as mock_renderer_class:
            mock_renderer = Mock()
            mock_renderer.render.return_value = expected_svg
            mock_renderer_class.return_value = mock_renderer
            
            result = quick_render(diagram_code)
            
            assert result == expected_svg
            mock_renderer.render.assert_called_once_with(diagram_code, format="svg")
    ...
    def test_png_rendering(self) -> None:
        ...
        with patch('mermaid_render.convenience.MermaidRenderer') as mock_renderer_class:
            mock_renderer = Mock()
            mock_renderer.render.return_value = expected_png
            mock_renderer_class.return_value = mock_renderer
            
            result = quick_render(diagram_code, format="png")
            
            assert result == expected_png
            mock_renderer.render.assert_called_once_with(diagram_code, format="png")
    ...
    def test_pdf_rendering(self) -> None:
        ...
        with patch('mermaid_render.convenience.MermaidRenderer') as mock_renderer_class:
            mock_renderer = Mock()
            mock_renderer.render.return_value = expected_pdf
            mock_renderer_class.return_value = mock_renderer
            
            result = quick_render(diagram_code, format="pdf")
            
            assert result == expected_pdf
            mock_renderer.render.assert_called_once_with(diagram_code, format="pdf")
    ...
    def test_with_theme(self) -> None:
        ...
        with patch('mermaid_render.convenience.MermaidRenderer') as mock_renderer_class:
            mock_renderer = Mock()
            mock_renderer.render.return_value = '<svg>test</svg>'
            mock_renderer_class.return_value = mock_renderer
            
            quick_render(diagram_code, theme="dark")
            
            mock_renderer_class.assert_called_once_with(theme="dark")
    ...
    def test_with_custom_config(self) -> None:
        ...
        with patch('mermaid_render.convenience.MermaidRenderer') as mock_renderer_class:
            ...
            mock_renderer_class.assert_called_once_with(config=config)
    ...
    def test_with_output_path(self, temp_dir: Any) -> None:
        ...
        with patch('mermaid_render.convenience.MermaidRenderer') as mock_renderer_class:
            ...
            assert result == expected_svg
            mock_renderer.save.assert_called_once_with(diagram_code, output_path, format="svg")
    ...
    def test_path_object_handling(self, temp_dir: Any) -> None:
        ...
        with patch('mermaid_render.convenience.MermaidRenderer') as mock_renderer_class:
            ...
            quick_render(diagram_code, output_path=output_path)
    ...
    def test_complex_diagram_code(self) -> None:
        ...
        with patch('mermaid_render.convenience.MermaidRenderer') as mock_renderer_class:
            mock_renderer = Mock()
            mock_renderer.render.return_value = '<svg>complex</svg>'
            mock_renderer_class.return_value = mock_renderer
            
            result = quick_render(complex_diagram)
            
            assert result == '<svg>complex</svg>'
            mock_renderer.render.assert_called_once_with(complex_diagram, format="svg")
    ...
    def test_unicode_diagram_code(self) -> None:
        ...
        with patch('mermaid_render.convenience.MermaidRenderer') as mock_renderer_class:
            mock_renderer = Mock()
            mock_renderer.render.return_value = '<svg>unicode</svg>'
            mock_renderer_class.return_value = mock_renderer
            
            result = quick_render(unicode_diagram)
            
            assert result == '<svg>unicode</svg>'
            mock_renderer.render.assert_called_once_with(unicode_diagram, format="svg")
...
class TestBackwardCompatibility:
    ...
    def test_render_alias(self) -> None:
        ...
        from mermaid_render.convenience import render, quick_render
...
class TestEdgeCases:
    ...
    def test_very_long_diagram_code(self) -> None:
        ...
        nodes = [f"    {chr(65+i)} --> {chr(65+i+1)}" for i in range(25)]  # A->B, B->C, etc.
        long_diagram = "flowchart TD\n" + "\n".join(nodes)
        
        with patch('mermaid_render.convenience.MermaidRenderer') as mock_renderer_class:
            mock_renderer = Mock()
            mock_renderer.render.return_value = '<svg>long</svg>'
            mock_renderer_class.return_value = mock_renderer
            
            result = quick_render(long_diagram)
            
            assert result == '<svg>long</svg>'
            mock_renderer.render.assert_called_once_with(long_diagram, format="svg")
    ...
    def test_none_values_handling(self) -> None:
        ...
        with patch('mermaid_render.convenience.MermaidRenderer') as mock_renderer_class:
            ...
            assert result == '<svg>test</svg>'
            mock_renderer_class.assert_called_once_with(theme=None, config=None)
