        ...
        for i, renderer_name in enumerate(renderer_chain):
            try:
                ...
                if required_capabilities:
                    if not all(renderer.has_capability(cap) for cap in required_capabilities):
                        self.logger.debug(
                            f"Renderer '{renderer_name}' missing required capabilities"
                        )
                        continue
                ...
                self.logger.warning(
                    f"Renderer '{renderer_name}' failed: {e}"
                )
