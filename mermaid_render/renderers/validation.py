...
class InputValidator:
    ...
    def validate(
        ...
        if len(mermaid_code) > self._limits['max_length']:
            errors.append(f"Input too long: {len(mermaid_code)} > {self._limits['max_length']} characters")
    ...
    def _check_basic_syntax(self, mermaid_code: str, diagram_type: Optional[str]) -> List[str]:
        ...
        if bracket_count != 0:
            issues.append(f"Unbalanced brackets: {bracket_count} extra opening brackets")
    ...
    def _check_complexity(self, mermaid_code: str) -> List[str]:
        ...
        edge_count = len(re.findall(r'-->', mermaid_code)) + len(re.findall(r'---', mermaid_code))
        ...
        if long_labels:
            warnings.append(f"Found {len(long_labels)} very long labels that may affect layout")
