...
class RendererConfigManager:
    ...
    def __init__(
        ...
        self.config_dir = Path(config_dir) if config_dir else Path.home() / ".mermaid_render"
    ...
    def save_renderer_config(
        ...
        try:
            ...
            if validate and renderer_name in self._schemas:
                validation_errors = self._validate_config(config, self._schemas[renderer_name])
                if validation_errors:
                    self.logger.error(
            ...
            self.logger.info(f"Saved configuration for {renderer_name} to {config_file}")
            return True
    ...
    def _load_env_config(self, renderer_name: str) -> Dict[str, Any]:
        ...
        if config:
            self.logger.debug(f"Loaded environment config for {renderer_name}: {list(config.keys())}")
