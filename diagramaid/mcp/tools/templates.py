"""
Template management MCP tools for diagramaid.

This module provides template creation and management tools with
...
async def create_from_template(
    ...
    parameters: Annotated[dict[str, Any], "Template parameters"],
    validate_params: Annotated[bool, "Validate template parameters"] = True,
    ...
    try:
        if ctx:
            await ctx.info(f"Creating diagram from template: {template_name}")
        ...
        try:
            ...
            if ctx:
                await ctx.error("Template functionality not available")
            return {
                "success": False,
        ...
        if ctx:
            await ctx.info("Template diagram created successfully")
        ...
        if ctx:
            await ctx.error(f"Template creation failed: {e}")
        return {
            "success": False,
            "error": str(e),
...
async def list_available_templates(
    template_name: Annotated[str | None, "Filter by template name"] = None,
    category: Annotated[str | None, "Filter by template category"] = None,
    include_builtin: Annotated[bool, "Include built-in templates"] = True,
    include_custom: Annotated[bool, "Include custom templates"] = True,
    ctx: Context | None = None,
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            return create_error_response(
                ...
                suggestions=[
                    "Install template support with: pip install diagramaid[templates]"
                ],
            )
...
async def get_template_details(
    template_name: Annotated[str, "Name or ID of the template"],
    ctx: Context | None = None,
) -> dict[str, Any]:
    """
    ...
    Example:
        >>> result = await get_template_details("software_architecture")
        >>> print(result["data"]["parameters"])  # Template parameters
        >>> print(result["data"]["examples"])    # Usage examples
    """
    try:
        if ctx:
            await ctx.info(f"Getting details for template: {template_name}")
...
async def create_custom_template(
    ...
    diagram_type: Annotated[str, "Type of diagram (flowchart, sequence, etc.)"],
    template_content: Annotated[str, "Jinja2 template content"],
    parameters: Annotated[dict[str, Any], "Parameter definitions"],
    description: Annotated[str, "Template description"] = "",
    author: Annotated[str | None, "Template author"] = None,
    tags: Annotated[list[str] | None, "Template tags for categorization"] = None,
    ctx: Context | None = None,
) -> dict[str, Any]:
    """
    ...
    try:
        if ctx:
            await ctx.info(f"Creating custom template: {name}")
