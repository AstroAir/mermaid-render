...
class TestCLIArgumentParsing:
    ...
    def test_format_choices(self, capsys: Any) -> None:
        ...
        for format_type in valid_formats:
            with patch.object(sys, 'argv', ['mermaid-render', 'test.mmd', '-f', format_type]):
                with patch('mermaid_render.cli.Path.exists', return_value=True):
                    with patch('mermaid_render.cli.Path.read_text', return_value='flowchart TD\n    A --> B'):
                        with patch('mermaid_render.cli.quick_render', return_value='<svg></svg>'):
                            result = main()
                            # Should not exit with argument error
                            assert result in [0, 1]  # May fail on rendering but not on parsing
...
class TestCLIFileOperations:
    ...
    def test_file_input_success(self, temp_dir: Any) -> None:
        ...
        with patch.object(sys, 'argv', ['mermaid-render', str(test_file)]):
            with patch('mermaid_render.cli.quick_render', return_value='<svg></svg>') as mock_render:
                result = main()
    ...
    def test_output_file_creation(self, temp_dir: Any) -> None:
        ...
        with patch.object(sys, 'argv', ['mermaid-render', str(test_file), '-o', str(output_file)]):
            with patch('mermaid_render.cli.quick_render', return_value='<svg></svg>') as mock_render:
                result = main()
...
class TestCLIValidationMode:
    ...
    def test_validation_success(self, temp_dir: Any, capsys: Any) -> None:
        ...
        with patch.object(sys, 'argv', ['mermaid-render', str(test_file), '--validate-only']):
    ...
    def test_validation_quiet_mode(self, temp_dir: Any, capsys: Any) -> None:
        ...
        with patch.object(sys, 'argv', ['mermaid-render', str(test_file), '--validate-only', '--quiet']):
            with patch('mermaid_render.utils.validate_mermaid_syntax') as mock_validate:
                mock_result = Mock()
                mock_result.is_valid = True
                mock_validate.return_value = mock_result
...
class TestCLIErrorScenarios:
    ...
    def test_rendering_error(self, temp_dir: Any, capsys: Any) -> None:
        ...
        with patch.object(sys, 'argv', ['mermaid-render', str(test_file)]):
            with patch('mermaid_render.cli.quick_render', side_effect=RenderingError("Rendering failed")):
                result = main()
    ...
    def test_validation_error(self, temp_dir: Any, capsys: Any) -> None:
        ...
        with patch.object(sys, 'argv', ['mermaid-render', str(test_file)]):
            with patch('mermaid_render.cli.quick_render', side_effect=ValidationError("Invalid syntax")):
                result = main()
    ...
    def test_output_required_for_binary_formats(self, temp_dir: Any, capsys: Any) -> None:
    ...
    def test_unexpected_exception(self, temp_dir: Any, capsys: Any) -> None:
        ...
        with patch.object(sys, 'argv', ['mermaid-render', str(test_file)]):
            with patch('mermaid_render.cli.quick_render', side_effect=Exception("Unexpected error")):
                result = main()
...
class TestCLIOutputFormatting:
    ...
    def test_quiet_mode_suppresses_messages(self, temp_dir: Any, capsys: Any) -> None:
        ...
        with patch.object(sys, 'argv', ['mermaid-render', str(test_file), '-o', str(output_file), '--quiet']):
            with patch('mermaid_render.cli.quick_render', return_value='<svg></svg>'):
                result = main()
    ...
    def test_theme_parameter_passed(self, temp_dir: Any) -> None:
        ...
        with patch.object(sys, 'argv', ['mermaid-render', str(test_file), '-t', 'dark']):
            with patch('mermaid_render.cli.quick_render', return_value='<svg></svg>') as mock_render:
                result = main()
